...
## 4. Públicos personalizados (Custom Audiences)

Leitura, criação, alteração e exclusão de públicos personalizados (Custom Audiences).

### `ads_get_ad_account_custom_audiences`

**🟢 Leitura**

Lista públicos personalizados da conta, com nome, subtipo e tamanho.

**Parâmetros:**

- `ad_account_id` (obrigatório)
- `subtype_filter` (opcional) — `CUSTOM`, `WEBSITE`, `LOOKALIKE`, `APP`, `ENGAGEMENT`, `OFFLINE_CONVERSION`
- `cursor` (opcional)
- `limit` (opcional; padrão 25, máx 100)
...
Detalhes de um público: tamanho, subtipo, `delivery_status`, `operation_status` e datas de criação/atualização.

**Parâmetros:**

- `custom_audience_id` (obrigatório)

**Atenção:**

- ⚠️ `delivery_status` INACTIVE/INVALID = público não pode ser usado para entrega.

### `ads_get_custom_audience_adsets`

**🟢 Leitura**

Lista os conjuntos de anúncios que usam determinado público no direcionamento.
...
Cria públicos de 5 subtipos: `CUSTOM` (lista de clientes/DFCA), `WEBSITE` (visitantes do site via pixel), `ENGAGEMENT` (engajamento com IG, Página, loja, marketplace, formulário de lead, canvas), `MOBILE_APP` (usuários do app) e `LOOKALIKE` (semelhante).

**Parâmetros:**

- `ad_account_id`, `name`, `subtype` (obrigatórios)
- `customer_file_source` (obrigatório p/ CUSTOM) — `USER_PROVIDED_ONLY`, `PARTNER_PROVIDED_ONLY`, `BOTH_USER_AND_PARTNER_PROVIDED`
- `rule` (obrigatório p/ WEBSITE/ENGAGEMENT/MOBILE_APP) — JSON string com inclusions/exclusions, event_sources, retention_seconds, templates
- `origin_audience_id` + `lookalike_ratio` (obrigatórios p/ LOOKALIKE) — ratio de 0.01 (1%) a 0.20 (20%); origem não pode ser outro lookalike
- `retention_days` (CUSTOM; 1–180, padrão 180)
- `is_value_based` (CUSTOM), `prefill` (padrão true), `audience_labels`, `description`

### `ads_update_custom_audience`

**🟠 Escrita (alteração)**

Atualiza metadados de público existente: nome, descrição, labels e, para públicos de site (WCA), a regra JSON.

**Parâmetros:**

- `custom_audience_id` (obrigatório) + ao menos 1 campo
- `name`, `description`, `audience_labels`, `rule` (só WCA)

**Atenção:**

- ⚠️ Retenção de DFCA não pode ser alterada.

### `ads_update_custom_audience_users`

**🟠 Escrita (alteração)**

Adiciona ou remove usuários de uma lista de clientes (DFCA) enviando PII com hash SHA-256 (aceita dados brutos ou já hasheados; normaliza e hasheia no servidor).

**Parâmetros:**

- `audience_id` (obrigatório)
- `schema` (obrigatório, lista ordenada) — `EMAIL`, `PHONE`, `FN`, `LN`, `ZIP`, `CT`, `ST`, `COUNTRY`, `DOB`, `GEN`, `MADID`, `EXTERN_ID`, `LOOKALIKE_VALUE`
- `data` (obrigatório) — linhas de usuários na ordem do schema
- `operation` (opcional) — `ADD` (padrão) ou `REMOVE`
- `customer_consent` (opcional), `debug_identifier` (opcional)

**Atenção:**

- ⚠️ `REMOVE` pode ser rejeitado se reduzir o público abaixo do mínimo de entrega de campanhas ativas.

### `ads_delete_custom_audience`

**🔴 Exclusão (destrutiva)**

Exclui um público PERMANENTEMENTE. Conjuntos de anúncios ativos que o usam são pausados automaticamente. Lookalikes derivados precisam ser excluídos antes.
...
## 5. Pixel — eventos e parâmetros

Gestão de regras de eventos de conversão do Pixel e seus extratores de parâmetros. Todas as ferramentas são batch (`items[]`) com opção `partial` (continuar após erro por item).

### `ads_pixel_event_read`

**🟢 Leitura**

Lê regras de eventos de conversão do Pixel (listar por pixel ou buscar por ID de regra). Não retorna os parâmetros do evento.

**Parâmetros:**

- `items` (obrigatório) — cada item: `pixel_id` (obrigatório), `event_rule_id` (opcional), `event_type` (opcional: Purchase, AddToCart, Lead etc.)
- `partial` (opcional, bool)

**Atenção:**

- ⚠️ Requer escopo `ads_read`, `ads_management` ou `business_management`.

### `ads_pixel_event_create`

**🟠 Escrita (criação)**

Cria regras de eventos padrão do Pixel (17 eventos padrão da Meta). `rule_type` `TOKENIZED_BUTTON_TEXT` (clique em botão) ou `URL` (carregamento de página). Eventos nascem `INACTIVE`.

**Parâmetros:**

- `items` — cada item: `pixel_id`, `event_type`, `rule_type`, `match_value`, `domain_uri` (obrigatórios); `operator` (padrão `EQUALS` p/ botão, `CONTAINS` p/ URL); `parameters[]` embutidos: `{parameter_type, extractor_type (CSS ou CONSTANT_VALUE), extractor_config_json}`
- `partial` (opcional)
...
Altera apenas o status da regra de evento: `ACTIVE` ou `INACTIVE` (ativar/pausar).

**Parâmetros:**

- `items` — cada item: `event_rule_id`, `status`
- `partial` (opcional)
...
Exclui regras de evento. Regras criadas na interface (USER_CONFIG) sofrem hard delete IRREVERSÍVEL; demais origens, soft delete (arquivadas). Parâmetros vinculados NÃO são excluídos em cascata.

**Parâmetros:**

- `items` — cada item: `event_rule_id`
- `partial` (opcional)
...
Lê extratores de parâmetros do Pixel (por pixel ou por ID de parâmetro), com filtros por domínio e tipo de evento.

**Parâmetros:**

- `items` — cada item: `pixel_id` (obrigatório), `parameter_id`, `domain_uri`, `event_type` (opcionais)
- `partial` (opcional)
...
Cria extratores de parâmetros: `CSS` (lê o innerText do elemento — não lê URL, atributos nem estado JS) ou `CONSTANT_VALUE` (valor fixo, ex.: `currency = BRL`).

**Parâmetros:**

- `items` — cada item: `pixel_id`, `domain_uri`, `event_type`, `extractor_type` (obrigatórios); `extractor_config_json`, `event_rule_id` (opcionais)
- `partial` (opcional)
...
Altera extrator existente; campos omitidos permanecem inalterados.

**Parâmetros:**

- `items` — cada item: `parameter_id` (obrigatório); `domain_uri`, `event_type`, `extractor_type`, `extractor_config_json`, `event_rule_id` (opcionais)
- `partial` (opcional)
...
Soft delete de extrator (arquivado). Não afeta a regra de evento vinculada.

**Parâmetros:**

- `items` — cada item: `parameter_id`
- `partial` (opcional)

---

## 6. Criação e gestão de campanhas

Todo o fluxo de criação: campanha → conjunto → criativo → anúncio → ativação. Entidades são sempre criadas em estado PAUSADO — o gasto só começa com `ads_activate_entity`.

### `ads_create_campaign`

**🟠 Escrita (criação)**

Cria campanha em PAUSADO. A Meta recomenda CBO (orçamento no nível da campanha): definir qualquer `campaign_daily/lifetime_budget` ativa CBO automaticamente.

**Parâmetros:**

- `ad_account_id`, `campaign_name`, `objective`, `buying_type` (obrigatórios)
- `objective` — apenas ODAX: `OUTCOME_AWARENESS`, `OUTCOME_TRAFFIC`, `OUTCOME_ENGAGEMENT`, `OUTCOME_LEADS`, `OUTCOME_SALES`, `OUTCOME_APP_PROMOTION` (objetivos legados são rejeitados)
- `buying_type` — `AUCTION` (padrão) ou `RESERVED`
- `campaign_daily_budget` OU `campaign_lifetime_budget` (centavos; excludentes; só CBO)
- `campaign_bid_strategy` — `LOWEST_COST_WITHOUT_CAP` (padrão), `LOWEST_COST_WITH_BID_CAP`, `COST_CAP`, `LOWEST_COST_WITH_MIN_ROAS`
- `campaign_spend_cap` (centavos), `campaign_start_time`/`stop_time` (ISO 8601)
- `special_ad_categories` (padrão `[]`) + `special_ad_category_country`
- `promoted_object` (JSON; exigido por alguns objetivos), `adlabels`, `budget_schedule_specs`, `source_campaign_id`, `is_skadnetwork_attribution` e outros

**Atenção:**

- ⚠️ A resposta traz `valid_optimization_goals` e `recommended_optimization_goal` para o conjunto seguinte.

### `ads_create_ad_set`

**🟠 Escrita (criação)**

Cria conjunto de anúncios em PAUSADO sob campanha existente, com dezenas de parâmetros de segmentação, orçamento, lances e agendamento.

**Parâmetros:**

- `ad_account_id`, `campaign_id`, `ad_set_name`, `billing_event`, `optimization_goal`, `targeting` (obrigatórios)
- `billing_event` — `IMPRESSIONS`, `LINK_CLICKS`, `POST_ENGAGEMENT`, `VIDEO_VIEWS`
- `optimization_goal` — 26 valores (`REACH`, `IMPRESSIONS`, `LINK_CLICKS`, `LANDING_PAGE_VIEWS`, `OFFSITE_CONVERSIONS`, `VALUE`, `LEAD_GENERATION`, `QUALITY_LEAD`, `CONVERSATIONS`, `APP_INSTALLS`, `THRUPLAY`, `VIDEO_VIEWS`, `PAGE_LIKES`, `POST_ENGAGEMENT` etc.), com compatibilidade validada por objetivo da campanha
- `targeting` (JSON) — nunca inventar IDs de interesse; broad: `{"geo_locations":{"countries":["BR"]}}`
- `daily_budget` / `lifetime_budget` (centavos) — SÓ em modo ABO (campanha sem orçamento próprio); `lifetime` exige `end_time`
- `bid_strategy` / `bid_amount` / `bid_constraints` — só ABO; `COST_CAP` e `BID_CAP` exigem `bid_amount`; `MIN_ROAS` exige `bid_constraints {roas_average_floor}`
- `promoted_object` (JSON) — OBRIGATÓRIO para `OFFSITE_CONVERSIONS`, `VALUE`, `LEAD_GENERATION`, `QUALITY_LEAD`, `APP_INSTALLS`, `IN_APP_VALUE`; ex.: `{"pixel_id":"123","custom_event_type":"PURCHASE"}`
- `destination_type` — obrigatório p/ metas de mensagem/perfil: `MESSENGER`, `WHATSAPP`, `INSTAGRAM_DIRECT`, `INSTAGRAM_PROFILE`, `FACEBOOK_PAGE`, `WEBSITE` etc.
- `dsa_beneficiary` / `dsa_payor` — obrigatórios ao segmentar países da UE
- opcionais: `adset_schedule` (day-parting), `attribution_spec`, `frequency_control_specs`, `placement`, `pacing_type`, `start_time`/`end_time`, spend caps, `is_dynamic_creative` e muitos outros

**Atenção:**

- ⚠️ Advantage+ Audience vem ativado por padrão: `age_min`/`max` viram sugestão; para trava rígida de idade, `targeting_automation.advantage_audience = 0`.
